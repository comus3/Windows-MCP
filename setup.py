...
# Setup configuration - only run if pyproject.toml doesn't exist or we're in legacy mode
if __name__ == "__main__":
    # Check if we should use setuptools only (no pyproject.toml)
    use_legacy_setup = (
        not (Path(__file__).parent / "pyproject.toml").exists() or
        "--legacy" in sys.argv
    )
    
    if use_legacy_setup:
        # Remove --legacy from argv if present
        ...
        setup(
            ...
            project_urls={
                "Bug Tracker": "https://github.com/comus3/Windows-MCP/issues",
                "Documentation": "https://mcp-windows.readthedocs.io",
                "Source Code": "https://github.com/comus3/Windows-MCP",
            },
        ...
        print("Using pyproject.toml for configuration. Run 'python setup.py --legacy' to use setup.py instead.")
